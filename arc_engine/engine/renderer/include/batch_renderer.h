...
//        void DrawWorldText(const Font& font,const std::string& text, const glm::vec3&pos);
    ...
    private:
        ...
        VertexArray vertex_array_text_;

        IndexBuffer index_buffer_;
        IndexBuffer index_buffer_text_;
        VertexBuffer vertex_buffer_;
        VertexBuffer vertex_buffer_text_;

        uint num_quads_;
        uint num_quads_text_;
        Vertex* vertices_;
        VertexText* vertices_text_;
        uint* indices_;

        TextureCont* textures_;
        Font** textures_text_;
        uint num_textures_, num_textures_text_;
    };
