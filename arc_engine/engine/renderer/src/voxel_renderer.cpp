...
void VoxelRenderer::DrawTest() {
  ...
  uint indices[36] = {
      ...
      0b011, 0b010, 0b110, 0b011, 0b110, 0b111, // top
      0b000, 0b110, 0b010, 0b000, 0b100, 0b110, // back
      0b001, 0b011, 0b111, 0b001, 0b111, 0b101, // first
      0b000, 0b011, 0b010, 0b000, 0b001, 0b011, // left
      0b000, 0b010, 0b011, 0b000, 0b011, 0b001, // right
  };
  vertex_buffer_.SetData(vertices, 8 * sizeof(VoxelVertex));
  index_buffer_.Setup(indices, 36);
  vertex_array_.set_index_buffer(index_buffer_);
  ...
  RenderCommand::DrawIndexed(vertex_array_, 36);
}
...
void VoxelRenderer::SetMaterials(Shader& shader,const std::vector<Material> &materials) {
  // materials
  std::string base = "u_materials[";
  std::string color = "].color";
  int i = 0;
...
void VoxelRenderer::DrawTextures(
  ...
  void VoxelRenderer::DrawTexture(Shader& shader, const Texture &texture, const glm::vec3 &pos, const glm::vec3 &rot, const glm::vec3 &size){
  // transform
  glm::mat4 transform = glm::mat4(1);
  transform = glm::translate(transform, pos);
  for (int i = 0; i < 3; ++i) {
    transform = glm::rotate(transform, glm::radians(rot[i]),
                            {i == 0, i == 1, i == 2});
  }
  transform = glm::scale(transform,
                         {size.x * texture.width(), size.y * texture.height(),
