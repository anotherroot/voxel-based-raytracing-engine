...
void RendererSystem::Setup(arc::Scene &scene) {
  ...
  combine_AO_shader_.Setup("/home/tilen/Projects/diploma/renderer/"
                     "engine/shaders/combine_AO_shader");
  bilateral_shader_.Setup("/home/tilen/Projects/diploma/renderer/"
  ...
  shadow_texture_.Init(256, 128, 256, {0.2, 0.2, 0.2}, {});
...
void RendererSystem::Update(arc::Scene &scene) {
  if (camera_.CheckIfMoved()) {
    ignore_old_AO_ = true;
    target_AO_fb_ = curr_AO_fb_;
    logi("changing camera");
  }
  if (1) {
    frame_number_++;
    ...
    frame_buffer_.Bind();
    arc::RendererAPI::SetClearColor({1.0, 0.0, 0.0, 0});
        ...
        for (auto [entity, transform, model] : view.each()) {
          // lights
          {
            ...
            for (auto [entity, lt, light] : lights.each()) {
              ...
              voxel_shader_.SetFloat(str + "radius", light.radius);
              i++;
            }
    ...
    if (1) {
      shadow_fb_.Bind();
      arc::RendererAPI::SetClearColor({0, 0, 0, 0});
      arc::RendererAPI::Clear();
      overlay_renderer_.Begin(shadow_shader_);
      {
        ...
        shadow_shader_.SetInt("u_num_rays",opt_.AO_num_rays);
        shadow_shader_.SetFloat2(
            "u_blue_noise_size",
            {blue_noise_tex_.width(), blue_noise_tex_.height()});
        // lights
        shadow_shader_.SetFloat3("u_ambient_color", opt_.ambient_color);
        shadow_shader_.SetFloat("u_diffuse_scalar", opt_.diffuse_scalar);
        shadow_shader_.SetFloat("u_ambient_scalar", opt_.ambient_scalar);
        shadow_shader_.SetFloat("u_specular_scalar", opt_.specular_scalar);
        shadow_shader_.SetFloat("u_AO_ray_dist", opt_.AO_ray_dist);
        {
          auto view =
              scene.registry()
                  .view<const arc::TransformComponent, arc::LightComponent>();
    ...
    if (1) { // copy ambient value
      AO_fbs_[curr_AO_fb_].Bind();
      arc::RendererAPI::SetClearColor({0, 0, 0, 0});
      arc::RendererAPI::Clear();
      ...
      AO_fbs_[curr_AO_fb_].Unbind();
      curr_AO_fb_ = (curr_AO_fb_+1)%10;
      if(ignore_old_AO_&&curr_AO_fb_ == target_AO_fb_){
        ignore_old_AO_=false; //TODO: uncomment
      }
    }
    ...
    if (1) { // combine AO
      ...
      arc::RendererAPI::SetClearColor({0, 0, 0, 0});
      arc::RendererAPI::Clear();
      overlay_renderer_.Begin(combine_AO_shader_);
      {
        /* combine_AO_shader_.SetInt("u_ignore_old", (int)ignore_old_AO_); */
        int num = !ignore_old_AO_ ? 10: (curr_AO_fb_ - target_AO_fb_ +10)%10;
        combine_AO_shader_.SetInt("u_num",glm::min(num, opt_.max_temporal));
        logi("u_num = {0}",glm::min(num, opt_.max_temporal));
        combine_AO_shader_.SetInt("u_AO[0]", 1);
        combine_AO_shader_.SetInt("u_AO[1]", 2);
        combine_AO_shader_.SetInt("u_AO[2]", 3);
        combine_AO_shader_.SetInt("u_AO[3]", 4);
        combine_AO_shader_.SetInt("u_AO[4]", 5);
        combine_AO_shader_.SetInt("u_AO[5]", 6);
        combine_AO_shader_.SetInt("u_AO[6]", 7);
        combine_AO_shader_.SetInt("u_AO[7]", 8);
        combine_AO_shader_.SetInt("u_AO[8]", 9);
        combine_AO_shader_.SetInt("u_AO[9]", 10);
        int curr = (curr_AO_fb_-1+10)%10;
        for (int i = 0; i < 10; ++i) {
          AO_fbs_[curr].Bind(0,i+1);
          curr = (curr-1+10)%10; 
        }
      }
      overlay_renderer_.End(combine_AO_shader_);
      single_fb_[0].Unbind();
    }

    if (1) { // bilateral filter

      single_fb_[1].Bind();
      arc::RendererAPI::SetClearColor({0.25, 0.25, 0.75, 1});
      arc::RendererAPI::Clear();
      overlay_renderer_.Begin(bilateral_shader_);
      {
        bilateral_shader_.SetFloat2(
            "u_window_size",
            {arc::Engine::window().width(), arc::Engine::window().height()});
        bilateral_shader_.SetFloat("u_q",opt_.AO_bilateral_tolerance);
        bilateral_shader_.SetInt("u_texture", 1);
        /* AO_fbs_[(curr_AO_fb_-1+10)%10].Bind(0,1); */
        single_fb_[0].Bind(0, 1);
      }
      overlay_renderer_.End(bilateral_shader_);
      single_fb_[1].Unbind();
    ...
    if(1){//FSAA
      ...
      overlay_renderer_.Begin(fsaa_shader_);
      {
        ...
        single_fb_[1].Bind(0, 1);
        fsaa_shader_.SetFloat2("u_resolution", 
            {arc::Engine::window().width(), arc::Engine::window().height()});
      }
