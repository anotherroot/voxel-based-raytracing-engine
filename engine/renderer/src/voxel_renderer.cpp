...
void VoxelRenderer::DrawBox(Shader &shader, const glm::mat4 &trans,
  ...
  auto transform = glm::scale(trans, size);
  shader.SetMat4("u_transform", transform);
  shader.SetFloat3("u_texture_size", {1, 1, 1});
  shader.SetFloat3("u_box_position", transform[3]);
  shader.SetFloat3("u_box_size", {size.x, size.y, size.z});
  shader.SetFloat3("u_materials[0].color", color);
  box_texture_.Bind(1);
  RendererAPI::DrawIndexed(vertex_array_, 36);
}
} // namespace arc
